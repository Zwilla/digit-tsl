 ...
 * nbItemPerPage : number of item per page (integer) listObj: data thead : array
 * with label & column size (label : string , size : "XXXpx"), displayHeading :
 * display or hide table header (true/false) }
 * 
 * Support : - Dynamic button with reflective call (see
 * modalChangesProdController.js for example) - HTML template
 * 
 */
digitTslWeb.directive('paginator', [ '$window', function($window) {
	return {
		...
		link : function($scope, element, attrs) {
			...
			// Render value as HTML: always true if not specified in options
			console.log($scope.options);
			if ($scope.options.renderHTML == undefined || $scope.options.renderHTML == null) {
				$scope.options.renderHTML = true;
			}
			...
			/**
			 * Create paginator Split listObj in X page & create dynamic footer
			 * pagination
			 */
