var NexU = function() {
	...
	function _initRunningPortAndVersion(scheme, successCallback, oldVersionDetectedCallback, notInstalledCallback, cannotLoadScriptCallback) {
		...
		_initRunningPortAndVersionRecursive(scheme, successCallback, oldVersionDetectedCallback, notInstalledCallback, cannotLoadScriptCallback, 0,
				portsStr.split(","));
	}

	function _initRunningPortAndVersionRecursive(scheme, successCallback, oldVersionDetectedCallback, notInstalledCallback, cannotLoadScriptCallback,
			index, ports) {
		port = ports[index].trim();
		$.get(scheme + "://localhost:" + port + "/nexu-info", function(data) {
			runningPort = port;
			runningVersion = data.version;
			if (runningVersion.indexOf(latestVersion) === 0) {
