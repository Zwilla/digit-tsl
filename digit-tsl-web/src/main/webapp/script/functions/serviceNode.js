	...
	if (service != null) {
		...
		if (service.typeIdentifier != undefined && service.typeIdentifier != null) {
			...
			if (service.typeIdentifier.indexOf("TrstSvd") != -1) {
				var typeIdentifier = service.typeIdentifier.split("https://uri.etsi.org/TrstSvd/Svctype/")[1];
			} else if (service.typeIdentifier.indexOf("TrstSvc") != -1) {
				var typeIdentifier = service.typeIdentifier.split("https://uri.etsi.org/TrstSvc/Svctype/")[1];
			}
			span = span + typeIdentifier + emptySpan;
		}
		if (service.currentStatus != undefined && service.currentStatus != null) {
			var currentStatus = service.currentStatus.split("https://uri.etsi.org/TrstSvc/TrustedList/Svcstatus/")[1];
