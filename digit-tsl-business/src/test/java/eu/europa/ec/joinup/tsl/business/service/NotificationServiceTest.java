...
public class NotificationServiceTest extends AbstractSpringTest {
    ...
    public void GtestCertificates() {
        ...
        // OK
        Assert.assertTrue(digitalIdValidator.isTwoCertificatesNotExpired(TLDigitalIdentityUtils.retrieveCertificates(notif.getMpPointer().getServiceDigitalId()), new Date(117, 8, 26, 12, 00)));
        Assert.assertTrue(digitalIdValidator.isShiftedPeriodValid(TLDigitalIdentityUtils.retrieveCertificates(notif.getMpPointer().getServiceDigitalId()), new Date(117, 8, 26, 12, 00)));

        // Still OK
        Assert.assertTrue(digitalIdValidator.isTwoCertificatesNotExpired(TLDigitalIdentityUtils.retrieveCertificates(notif.getMpPointer().getServiceDigitalId()), new Date(125, 0, 1, 12, 00)));
        Assert.assertTrue(digitalIdValidator.isShiftedPeriodValid(TLDigitalIdentityUtils.retrieveCertificates(notif.getMpPointer().getServiceDigitalId()), new Date(124, 9, 1, 12, 00)));

        // Less than 4 months before the expiration of shift period
        Assert.assertFalse(digitalIdValidator.isShiftedPeriodValid(TLDigitalIdentityUtils.retrieveCertificates(notif.getMpPointer().getServiceDigitalId()), getVerifDay(new Date(125, 0, 1, 12, 00))));
