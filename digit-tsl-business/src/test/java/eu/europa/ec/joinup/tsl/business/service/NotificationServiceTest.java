...
public class NotificationServiceTest extends AbstractSpringTest {
  ...
  private final NotificationPointers notification = new NotificationPointers();
  @Autowired private NotificationService notificationService;

  @Autowired private NotificationRepository notificationRepository;

  @Autowired private NotificationJaxbService notificationJaxbService;

  @Autowired private DigitalIdentityValidator digitalIdValidator;

  @Autowired private TLRepository tlRepo;

  @Autowired private CountryService countryService;

  @Autowired private TLLoader tlLoader;
  ...
  public void BcreateDraftFromBinaries() {
    ...
    try {
      DBNotification dbNotif =
          notificationService.persistNotification(
              out.toByteArray(), out.toByteArray(), draftStoreId);
      Assert.assertNotNull(dbNotif);
      Assert.assertEquals(dbNotif.getDraftStoreId(), draftStoreId);
      Assert.assertEquals(dbNotif.getTerritory().getCodeTerritory(), "BE");
  ...
  public void DsetContactChange() {
    DBNotification dbNotif = notificationRepository.findOne(1);
    Assert.assertNotNull(dbNotif);
      Assert.assertFalse(dbNotif.isContactChange());
    notificationService.updateStatus(dbNotif, NotificationStatus.VALIDATED);
    DBNotification dbNotifUpdated = notificationRepository.findOne(1);
    Assert.assertEquals(NotificationStatus.VALIDATED, dbNotifUpdated.getStatus());
  }
  ...
  public void FuploadNotif() throws Exception {
    ...
    NotificationPointers prodPointer =
        notificationService.getNotification(memberNotif.getSchemeContact().getTerritory());
    NotificationPointers notifPointer =
        TLNotificationMapper.mapXMLNotificationObjToDTONotification(memberNotif, prodPointer);
  ...
  public void GtestCertificates() {
    ...
    // OK
    Assert.assertTrue(
        digitalIdValidator.isTwoCertificatesNotExpired(
            TLDigitalIdentityUtils.retrieveCertificates(notif.getMpPointer().getServiceDigitalId()),
            new Date(117, Calendar.SEPTEMBER, 26, 12, 0)));
    Assert.assertTrue(
        digitalIdValidator.isShiftedPeriodValid(
            TLDigitalIdentityUtils.retrieveCertificates(notif.getMpPointer().getServiceDigitalId()),
            new Date(117, Calendar.SEPTEMBER, 26, 12, 0)));
    ...
    Assert.assertTrue(
        digitalIdValidator.isTwoCertificatesNotExpired(
            TLDigitalIdentityUtils.retrieveCertificates(notif.getMpPointer().getServiceDigitalId()),
            new Date(125, Calendar.JANUARY, 1, 12, 0)));
    Assert.assertTrue(
        digitalIdValidator.isShiftedPeriodValid(
            TLDigitalIdentityUtils.retrieveCertificates(notif.getMpPointer().getServiceDigitalId()),
            new Date(124, Calendar.OCTOBER, 1, 12, 0)));

    // Less than 4 months before the expiration of shift period
    Assert.assertFalse(
        digitalIdValidator.isShiftedPeriodValid(
            TLDigitalIdentityUtils.retrieveCertificates(notif.getMpPointer().getServiceDigitalId()),
            getVerifDay(new Date(125, Calendar.JANUARY, 1, 12, 0))));
  }
