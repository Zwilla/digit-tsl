...
/**
 * Convenience class for generating a Map whose keys are values of
 * SchemeTerritory components and whose values are treeSets of ByteArray
 * instances that encapsulate DER-encoded X509 certificates that, according to
 * the ECLOL may sign the Trusted List of the country indicated by the key value
 *
 */
public class EUMSTLsSigningCertsAsInECLOLReader {
	 ...
	 * Constructor
	 *
	 * @param arg inputstream for the file where the ECLOL may be accessed
	 */
	...
	/**
	 * Method that generates the Map whose keys are values of SchemeTerritory
	 * components and whose values are treeSets of ByteArray instances that
	 * encapsulate DER-encoded X509 certificates that, according to the ECLOL
	 * may sign the Trusted List of the country indicated by the key value
	 *
	 * @return the aforementioned map
	 * @throws Exception if any problem occurs while processing the ECLOL
	 */
	public Map<String, TreeSet<ByteArray>> getEUMSTLsSigingCertsByCountryCode()
			throws Exception {
	...
	private void addCertsFromPointer(Map<String, TreeSet<ByteArray>> result, OtherTSLPointerType ptr,
			int numPointer) {
	...
	private void getCertificatesForTerritory(Map<String, TreeSet<ByteArray>> result,
			String schemeTerritory, ServiceDigitalIdentityListType sdisList,
			int numPointer) {
		...
		if (!found) {
			System.out.println("No X509 certificates found for pointer to "
					+ "TL from " + schemeTerritory + " in pointer number "
					+ numPointer);
		}
	}
	...
	protected Element getRootElementFromInputStream(InputStream input)
			throws Exception {
